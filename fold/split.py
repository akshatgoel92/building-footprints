...
def get_dest_files(train_target, val_target, train_frames, val_frames):
    ...
    ------------------------
    """
    train_dest_frames = [common.get_s3_paths(train_target, os.path.basename(img)) for img in train_frames]
    val_dest_frames = [common.get_s3_paths(val_target, os.path.basename(img)) for img in val_frames]
